...
export function NutritionTemplateEditorScreen() {
  ...
  return <Screen withinTabs style={styles.page}><TopBar eyebrow={creating ? 'NOVO PRESET' : 'EDITAR PRESET'} title={creating ? 'Preset de refeição' : query.data!.name} onBack={() => dirty ? Alert.alert('Descartar alterações?', 'O preset continuará como estava antes desta edição.', [{ text: 'Continuar editando', style: 'cancel' }, { text: 'Descartar', style: 'destructive', onPress: () => router.back() }]) : router.back()} /><Text style={styles.copy}>Monte uma refeição reutilizável, como “Café com ovos” ou “Café com tapioca”. Cada aplicação adiciona uma cópia independente ao plano do aluno.</Text><NutritionEditor key={id} mealTemplate initialValue={initial} pending={mutation.isPending} error={mutation.error?.message} submitLabel={creating ? 'Criar preset de refeição' : 'Salvar preset'} onDirtyChange={handleDirtyChange} onSubmit={(input) => { const meal = input.meals[0]; mutation.mutate({ name: meal.name, notes: meal.notes, foods: meal.foods }, { onSuccess: done }); }} />{!creating ? <><Button variant="secondary" loading={duplicate.isPending} onPress={() => duplicate.mutate(id, { onSuccess: (copy) => { feedback.success(); router.replace({ pathname: '/trainer/nutrition/templates/[id]', params: { id: copy.id } }); } })}>Duplicar preset</Button><Button variant="ghost" loading={remove.isPending} onPress={() => Alert.alert('Excluir preset?', 'As refeições já adicionadas aos alunos não serão alteradas.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Excluir', style: 'destructive', onPress: () => remove.mutate(id, { onSuccess: done }) }])}>Excluir preset</Button></> : null}</Screen>;
}
