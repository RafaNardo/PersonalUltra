...
export function nutritionInput(draft: NutritionDraft, mealTemplate = false): TrainerNutritionInput { const firstMeal = draft.meals[0]; return { name: (mealTemplate ? firstMeal?.name : draft.name)?.trim() ?? '', notes: (mealTemplate ? firstMeal?.notes : draft.notes)?.trim() ?? '', meals: draft.meals.map((meal, index) => ({ name: meal.name.trim(), notes: meal.notes.trim(), sequence: index + 1, foods: meal.foods.map((food, foodIndex) => ({ foodName: food.foodName.trim(), quantity: food.unit === 'livre' ? 1 : quantity(food.quantity), unit: food.unit, sequence: foodIndex + 1 })) })) }; }
