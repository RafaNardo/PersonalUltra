...
const units: NutritionQuantityUnit[] = ['g', 'ml', 'unidade', 'fatia', 'colher', 'dose', 'porção', 'livre'];
let counter = 0;
...
type DailyGoalsDraft = { calories: string; proteinGrams: string; carbohydratesGrams: string; fatGrams: string };
export type NutritionDraft = { name: string; notes: string; dailyGoals: DailyGoalsDraft; meals: MealDraft[] };
...
export function nutritionDraft(value?: { name: string; notes: string; dailyGoals?: { calories?: number; proteinGrams?: number; carbohydratesGrams?: number; fatGrams?: number }; meals?: TrainerNutritionMeal[] } | null): NutritionDraft {
  if (!value) return { name: '', notes: '', dailyGoals: { calories: '', proteinGrams: '', carbohydratesGrams: '', fatGrams: '' }, meals: [emptyMeal()] };
  return { name: value.name, notes: value.notes, dailyGoals: { calories: value.dailyGoals?.calories?.toString() ?? '', proteinGrams: value.dailyGoals?.proteinGrams?.toString() ?? '', carbohydratesGrams: value.dailyGoals?.carbohydratesGrams?.toString() ?? '', fatGrams: value.dailyGoals?.fatGrams?.toString() ?? '' }, meals: [...(value.meals ?? [])].sort((a, b) => a.sequence - b.sequence).map((meal) => ({ clientId: localId('meal'), name: meal.name, notes: meal.notes, foods: [...meal.foods].sort((a, b) => a.sequence - b.sequence).map((food) => ({ clientId: localId('food'), foodName: food.foodName, quantity: String(food.quantity), unit: food.unit })) })) };
}
export function nutritionMealTemplateDraft(value?: Pick<NutritionMealTemplate, 'name' | 'notes' | 'foods'> | null): NutritionDraft {
  if (!value) return { name: '', notes: '', dailyGoals: { calories: '', proteinGrams: '', carbohydratesGrams: '', fatGrams: '' }, meals: [emptyMeal()] };
  return { name: '', notes: '', dailyGoals: { calories: '', proteinGrams: '', carbohydratesGrams: '', fatGrams: '' }, meals: [{ clientId: localId('meal'), name: value.name, notes: value.notes, foods: [...(value.foods ?? [])].sort((a, b) => a.sequence - b.sequence).map((food) => ({ clientId: localId('food'), foodName: food.foodName, quantity: String(food.quantity), unit: food.unit })) }] };
}
const quantity = (value: string) => Number(value.replace(',', '.'));
const optionalQuantity = (value: string) => value.trim() ? quantity(value) : undefined;
export function nutritionInput(draft: NutritionDraft, mealTemplate = false): TrainerNutritionInput { const firstMeal = draft.meals[0]; return { name: (mealTemplate ? firstMeal?.name : draft.name)?.trim() ?? '', notes: (mealTemplate ? firstMeal?.notes : draft.notes)?.trim() ?? '', ...(mealTemplate ? {} : { dailyGoals: { calories: optionalQuantity(draft.dailyGoals.calories), proteinGrams: optionalQuantity(draft.dailyGoals.proteinGrams), carbohydratesGrams: optionalQuantity(draft.dailyGoals.carbohydratesGrams), fatGrams: optionalQuantity(draft.dailyGoals.fatGrams) } }), meals: draft.meals.map((meal, index) => ({ name: meal.name.trim(), notes: meal.notes.trim(), sequence: index + 1, foods: meal.foods.map((food, foodIndex) => ({ foodName: food.foodName.trim(), quantity: food.unit === 'livre' ? 1 : quantity(food.quantity), unit: food.unit, sequence: foodIndex + 1 })) })) }; }
function validate(draft: NutritionDraft, mealTemplate: boolean) {
  ...
  if (!mealTemplate) Object.entries(draft.dailyGoals).forEach(([key, value]) => { if (value.trim() && (!Number.isFinite(quantity(value)) || quantity(value) < 0 || (key === 'calories' ? quantity(value) > 20000 : quantity(value) > 2000))) errors.push('Informe metas diárias válidas.'); });
  ...
  draft.meals.forEach((meal, mealIndex) => {
    ...
    meal.foods.forEach((food, foodIndex) => {
      ...
      if (food.unit !== 'livre' && (!Number.isFinite(quantity(food.quantity)) || quantity(food.quantity) <= 0 || quantity(food.quantity) > 10000)) errors.push(`Informe uma quantidade maior que zero e até 10000 em ${mealName}, item ${foodIndex + 1}.`);
    });
  });
  return errors;
}

export function NutritionEditor({ initialValue, submitLabel, pending, error, onSubmit, onDirtyChange, mealTemplate = false, summaryOnly = false }: { initialValue: NutritionDraft; submitLabel: string; pending: boolean; error?: string; onSubmit: (input: TrainerNutritionInput) => void; onDirtyChange?: (dirty: boolean) => void; mealTemplate?: boolean; summaryOnly?: boolean }) {
  ...
  return <>
    {!mealTemplate ? <Card style={styles.card}><Text style={styles.title}>Identificação</Text><Field label="Nome" value={draft.name} onChange={(name) => setDraft((current) => ({ ...current, name }))} placeholder="Ex.: Alimentação para rotina de treinos" /><Field label="Orientações gerais (opcional)" value={draft.notes} onChange={(notes) => setDraft((current) => ({ ...current, notes }))} placeholder="Contexto e orientações" multiline /><Text style={styles.sectionTitle}>Metas diárias (opcional)</Text><Text style={styles.metaCopy}>Informe apenas as referências que quiser compartilhar. O app não calcula nem ajusta estes valores.</Text><View style={styles.goalGrid}><Field label="Calorias" value={draft.dailyGoals.calories} onChange={(calories) => setDraft((current) => ({ ...current, dailyGoals: { ...current.dailyGoals, calories } }))} placeholder="Ex.: 2200" keyboardType="decimal-pad" /><Field label="Proteínas (g)" value={draft.dailyGoals.proteinGrams} onChange={(proteinGrams) => setDraft((current) => ({ ...current, dailyGoals: { ...current.dailyGoals, proteinGrams } }))} placeholder="Ex.: 140" keyboardType="decimal-pad" /><Field label="Carboidratos (g)" value={draft.dailyGoals.carbohydratesGrams} onChange={(carbohydratesGrams) => setDraft((current) => ({ ...current, dailyGoals: { ...current.dailyGoals, carbohydratesGrams } }))} placeholder="Ex.: 260" keyboardType="decimal-pad" /><Field label="Gorduras (g)" value={draft.dailyGoals.fatGrams} onChange={(fatGrams) => setDraft((current) => ({ ...current, dailyGoals: { ...current.dailyGoals, fatGrams } }))} placeholder="Ex.: 70" keyboardType="decimal-pad" /></View></Card> : null}
    {!summaryOnly && draft.meals.map((meal, mealIndex) => <Card key={meal.clientId} style={styles.card}>
      ...
      {meal.foods.map((food, foodIndex) => <View key={food.clientId} style={styles.food}>
        ...
        <Field label="Alimento" value={food.foodName} onChange={(foodName) => updateMeal(mealIndex, (m) => ({ ...m, foods: m.foods.map((f, i) => i === foodIndex ? { ...f, foodName } : f) }))} placeholder="Ex.: Arroz integral" />
        <Text style={styles.label}>Medida</Text><View style={styles.units}>{units.map((unit) => <Pressable key={unit} onPress={() => updateMeal(mealIndex, (m) => ({ ...m, foods: m.foods.map((f, i) => i === foodIndex ? { ...f, unit, quantity: unit === 'livre' ? '1' : f.quantity } : f) }))} style={[styles.unit, food.unit === unit && styles.unitSelected]}><Text style={[styles.unitText, food.unit === unit && styles.unitTextSelected]}>{unit}</Text></Pressable>)}</View>
        {food.unit === 'livre' ? <Text style={styles.freeCopy}>Sem quantidade fixa — ideal para itens como salada.</Text> : <Field label="Quantidade" value={food.quantity} onChange={(value) => updateMeal(mealIndex, (m) => ({ ...m, foods: m.foods.map((f, i) => i === foodIndex ? { ...f, quantity: value } : f) }))} placeholder="Ex.: 120" keyboardType="decimal-pad" />}
      </View>)}
    ...
    {!summaryOnly && !mealTemplate ? <Button variant="secondary" disabled={draft.meals.length >= 20} onPress={() => setDraft((c) => ({ ...c, meals: [...c.meals, emptyMeal()] }))}>+ Adicionar refeição</Button> : null}
