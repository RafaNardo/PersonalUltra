...
function PreviewExercise({ exercise }: { exercise: StudentWorkoutPreview['exercises'][number] }) {
  return <Card style={styles.exerciseCard}>
    ...
    <View style={styles.prescription}><Prescription label={exercise.trackingMode === 'Duration' ? 'Blocos' : 'Séries'} value={String(exercise.sets)} /><Prescription label={exercise.trackingMode === 'Duration' ? 'Tempo' : 'Repetições'} value={exercise.trackingMode === 'Duration' ? formatDuration(exercise.targetDurationSeconds) : `${exercise.repetitionsMin}–${exercise.repetitionsMax}`} /><Prescription label="Descanso" value={`${exercise.restSeconds}s`} /></View>
...
const styles = StyleSheet.create({ page: { paddingVertical: spacing.xl, gap: spacing.lg }, headingMeta: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: spacing.sm }, copy: { ...typography.bodyMD, color: colors.textSecondary, lineHeight: 21 }, intro: { ...typography.bodyLG, color: colors.titaniumLight, lineHeight: 24 }, meta: { ...typography.caption, color: colors.titanium, flexShrink: 1 }, exerciseCard: { gap: spacing.md }, exerciseHeader: { flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: spacing.md }, exerciseImage: { width: 88, height: 88, borderRadius: radius.sm, backgroundColor: colors.surfaceElevated }, imageFallback: { width: 88, height: 88, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surfaceElevated }, sequence: { ...typography.headingMD, color: colors.primary }, identity: { flex: 1, minWidth: 160, gap: spacing.xxs }, title: { ...typography.headingMD, color: colors.textPrimary }, context: { ...typography.caption, color: colors.titanium }, prescription: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs }, prescriptionItem: { flexGrow: 1, flexBasis: 92, gap: spacing.xxs, padding: spacing.sm, borderRadius: radius.sm, backgroundColor: colors.surfaceElevated }, prescriptionLabel: { ...typography.caption, color: colors.textMuted }, prescriptionValue: { ...typography.bodyLG, color: colors.textPrimary }, detail: { gap: spacing.xs, paddingTop: spacing.xs, borderTopWidth: 1, borderTopColor: colors.border }, detailLabel: { ...typography.caption, color: colors.primary, letterSpacing: .6 } });
