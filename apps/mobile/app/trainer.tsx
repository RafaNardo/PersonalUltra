...
export default function TrainerEntryScreen() {
  ...
  return <Screen style={styles.page}>
    ...
    <Button onPress={() => router.push('/trainer/invite')}>Convidar novo aluno</Button>
    <Button variant="secondary" onPress={() => router.push('/trainer/training')}>Gerenciar treinos</Button>
    <View style={styles.section}><View style={styles.sectionHeader}><Text style={styles.sectionTitle}>Alunos recentes</Text><Button variant="ghost" style={styles.viewAll} onPress={() => router.push('/trainer/students')}>Ver todos</Button></View>{data.recentStudents.length === 0 ? <Card><Text style={styles.copy}>Seus próximos alunos aparecerão aqui.</Text></Card> : data.recentStudents.map((student) => <Pressable key={student.studentId} accessibilityRole="button" accessibilityLabel={`Abrir ${student.firstName} ${student.lastName}`} accessibilityHint="Abre os detalhes do aluno" onPress={() => router.push({ pathname: '/trainer/students/[id]', params: { id: student.studentId } })} style={({ pressed }) => pressed && styles.pressed}><Card style={styles.student}><View style={styles.studentHeader}><Text style={styles.studentName}>{student.firstName} {student.lastName}</Text><Tag tone={student.anamnesisStatus === 'Completed' ? 'success' : 'neutral'}>{anamnesisLabel(student.anamnesisStatus)}</Tag></View>{student.email && <Text style={styles.studentEmail}>{student.email}</Text>}<Text style={styles.detailLink}>Ver detalhes ›</Text></Card></Pressable>)}</View>
...
const styles = StyleSheet.create({
  ...
  section: { gap: spacing.sm }, sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, sectionTitle: { ...typography.headingMD, color: colors.textPrimary }, viewAll: { minHeight: 36, paddingHorizontal: spacing.sm }, pressed: { opacity: .78 }, student: { gap: spacing.xs }, studentHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm }, studentName: { ...typography.headingMD, color: colors.textPrimary, flex: 1 }, studentEmail: { ...typography.bodyMD, color: colors.textSecondary }, detailLink: { ...typography.caption, color: colors.primary, marginTop: spacing.xs }, activity: { gap: spacing.xs }, activityTitle: { ...typography.bodyLG, color: colors.textPrimary },
});
