...
export default function TrainerInviteScreen() {
  ...
  const copyInvite = async () => {
    ...
    Alert.alert(
      "Convite copiado",
      "A mensagem completa está na área de transferência.",
    );
  };
  ...
  const copyCodeOnly = async () => {
    ...
    Alert.alert(
      "Código copiado",
      "O código de convite está na área de transferência.",
    );
  };
  ...
  return (
    <Screen withinTabs style={styles.page}>
      ...
      <Text style={styles.copy}>
        Compartilhe o código para que o aluno faça o cadastro e preencha a
        anamnese. O convite expira em sete dias.
      </Text>
      <Card style={styles.card}>
        ...
        <PhoneInput
          ...
          placeholderTextColor={colors.textMuted}
          accessibilityLabel="Telefone para WhatsApp do aluno"
          style={styles.input}
        />
        <Button loading={createInvite.isPending} onPress={() => void create()}>
      ...
      {inviteCode && (
        <Card style={styles.result}>
          ...
          <Button variant="secondary" onPress={() => void copyCodeOnly()}>
          ...
          {!canSendWhatsApp && (
            <Text style={styles.hint}>
              Informe um telefone válido com DDD para enviar pelo WhatsApp.
            </Text>
          )}
        </Card>
      )}
...
const styles = StyleSheet.create({
  ...
  copy: { ...typography.bodyMD, color: colors.textSecondary, lineHeight: 22 },
  card: { gap: spacing.md },
  ...
  resultTitle: { ...typography.headingMD, color: colors.textPrimary },
  code: {
