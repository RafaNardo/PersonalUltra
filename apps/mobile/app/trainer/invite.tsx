...
export default function TrainerInviteScreen() {
  ...
  const copyInvite = async () => {
    ...
    Alert.alert('Link copiado', 'Agora é só enviar o convite ao aluno.');
  };
  ...
  const sendWhatsApp = async () => {
    ...
    await Linking.openURL(`https://wa.me/${phoneDigits}?text=${encodeURIComponent(message)}`);
  };
  ...
  return <Screen style={styles.page}>
    ...
    {inviteUrl && <Card style={styles.result}><Text style={styles.resultTitle}>Convite pronto</Text><Text style={styles.copy}>Envie o link diretamente ou use o WhatsApp para incluir as orientações de instalação.</Text><TextInput value={inviteUrl} editable={false} selectTextOnFocus accessibilityLabel="Link de convite para copiar" style={styles.link} /><Button variant="secondary" onPress={() => void copyInvite()}>Copiar link</Button><Button variant="secondary" disabled={!canSendWhatsApp} onPress={() => void sendWhatsApp()}>Enviar pelo WhatsApp</Button>{!canSendWhatsApp && <Text style={styles.hint}>Informe um telefone válido com DDD para enviar pelo WhatsApp.</Text>}</Card>}
  </Screen>;
}
