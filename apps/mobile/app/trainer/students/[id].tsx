  ...
  return <Screen withinTabs style={styles.page}>
    ...
    {section === 'nutrition' && <>
      ...
      {nutrition.isLoading ? <Card><Text style={styles.copy}>Carregando alimentação…</Text></Card> : nutrition.isError ? <Card style={styles.card}><Text style={styles.errorText}>Não foi possível carregar a alimentação.</Text><Button variant="secondary" onPress={() => nutrition.refetch()}>Tentar novamente</Button></Card> : !nutrition.data ? <EmptyState status="ALIMENTAÇÃO PENDENTE" symbol="+" title="Prepare a primeira alimentação deste aluno." message="Escolha um preset ou comece do zero e revise tudo antes de disponibilizar." actionLabel="Adicionar alimentação" onAction={() => router.push({ pathname: '/trainer/students/[studentId]/nutrition/add', params: { studentId: id! } })} /> : <>
        <Card style={styles.card}><Text style={styles.eyebrow}>PLANO ATUAL</Text><Text style={styles.cardTitle}>{nutrition.data.name}</Text>{nutrition.data.notes ? <Text style={styles.copy}>{nutrition.data.notes}</Text> : null}<Text style={styles.planMeta}>Responsável: {nutrition.data.responsibleTrainerName} · atualizado em {new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short' }).format(new Date(nutrition.data.updatedAt))}</Text></Card>
        {[...nutrition.data.meals].sort((a, b) => a.sequence - b.sequence).map((meal) => <Card key={meal.id} style={styles.mealCard}><Text style={styles.eyebrow}>REFEIÇÃO {meal.sequence}</Text><Text style={styles.cardTitle}>{meal.name}</Text>{meal.notes ? <Text style={styles.copy}>{meal.notes}</Text> : null}{[...meal.foods].sort((a, b) => a.sequence - b.sequence).map((food) => <View key={food.id} style={styles.foodRow}><Text style={styles.foodName}>{food.foodName}</Text><Text style={styles.foodQuantity}>{food.quantity} {food.unit}</Text></View>)}</Card>)}
        <Button onPress={() => router.push({ pathname: '/trainer/students/nutrition/[id]', params: { id: id! } })}>Editar plano</Button>
        <Button variant="secondary" onPress={() => router.push({ pathname: '/trainer/students/[studentId]/nutrition/from-template', params: { studentId: id! } })}>Substituir usando preset</Button>
      </>}
    </>}
  </Screen>;
}
