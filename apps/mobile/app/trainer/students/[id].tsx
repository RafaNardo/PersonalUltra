...
function NutritionMealCard({ meal, index, count, busy, onMove, onEdit }: { meal: TrainerNutritionMeal; index: number; count: number; busy: boolean; onMove: (to: number) => void; onEdit: () => void }) {
  return <Card style={styles.mealItem}>
    <Pressable accessibilityRole="button" accessibilityLabel={`Editar refeição ${meal.name}`} onPress={onEdit} style={({ pressed }) => [styles.mealPressable, pressed && styles.pressed]}>
      ...
      <Text style={styles.openHint}>Editar refeição</Text>
    </Pressable>
