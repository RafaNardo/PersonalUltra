...
export default function TrainerStudentDetailScreen() {
  ...
  return <Screen style={styles.page}>
    ...
    {section === 'training' && <>
      ...
      <View style={styles.workoutCreationActions}><Button onPress={() => router.push({ pathname: '/trainer/students/[studentId]/workouts/new', params: { studentId: id! } })}>Criar treino do zero</Button><Button variant="secondary" onPress={() => router.push({ pathname: '/trainer/students/[studentId]/workouts/from-template', params: { studentId: id! } })}>Usar um modelo</Button></View>
      ...
      {!workouts.isLoading && !workouts.isError && workouts.data?.length === 0 && <EmptyState status="PRIMEIRA PRESCRIÇÃO" symbol="+" title="Prepare o primeiro treino deste aluno." message="Comece do zero para montar uma prescrição exclusiva ou use um modelo pronto acima." actionLabel="Criar treino do zero" onAction={() => router.push({ pathname: '/trainer/students/[studentId]/workouts/new', params: { studentId: id! } })} />}
      {workouts.data?.map((workout) => <Pressable key={workout.id} accessibilityRole="button" accessibilityLabel={`Abrir treino ${workout.name}`} accessibilityHint="Mostra os exercícios prescritos para este aluno" onPress={() => router.push({ pathname: '/trainer/students/[studentId]/workouts/[workoutId]', params: { studentId: id!, workoutId: workout.id } })} style={({ pressed }) => pressed && styles.pressed}>
        <Card style={styles.workoutCard}>
          <View style={styles.workoutHeader}><Text style={styles.workoutName}>{workout.name}</Text>{workout.exerciseCount === 0 ? <Tag tone="primary">EM CONSTRUÇÃO</Tag> : workout.isRecommended ? <Tag tone="success">RECOMENDADO</Tag> : <Tag tone="neutral">ATIVO</Tag>}</View>
