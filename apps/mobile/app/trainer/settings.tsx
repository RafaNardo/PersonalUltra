...
import {
  useTrainerPrescriptionSettings,
  useUpdateTrainerPrescriptionSettings,
} from "@/src/features/trainer/settings/hooks";
import {
  hasPrescriptionErrors,
  prescriptionDraftFromDefaults,
  validateExercisePrescription,
  type ExercisePrescriptionDraft,
} from "@/src/features/trainer/training/prescription";
...
export default function TrainerSettingsScreen() {
  ...
  const errors = useMemo(
    () => (draft ? validateExercisePrescription(draft) : {}),
    [draft],
  );
  if (settings.isLoading)
    return <LoadingView message="Carregando suas configurações…" />;
  if (settings.isError)
    return (
      <ErrorView
  ...
  return (
    <Screen withinTabs style={styles.page}>
      ...
      <Text style={styles.intro}>
        Defina um ponto de partida para novos exercícios. Você ainda poderá
        ajustar cada prescrição antes de publicá-la.
      </Text>
        ...
        <View style={styles.sectionHeader}>
          <View style={styles.sectionText}>
            ...
            <Text style={styles.sectionTitle}>
              Configuração inicial dos exercícios
            </Text>
          </View>
        ...
        <View style={styles.rangeRow}>
          <View style={styles.rangeField}>
            <Stepper
              label="Rep. mínimas"
              helper="De 1 a 100"
              value={draft.repetitionsMin}
              min={1}
              max={100}
              onChange={(value) => update("repetitionsMin", value)}
              error={errors.repetitionsMin}
              compact
            />
          </View>
          <View style={styles.rangeField}>
            <Stepper
              label="Rep. máximas"
              helper="De 1 a 100"
              value={draft.repetitionsMax}
              min={1}
              max={100}
              onChange={(value) => update("repetitionsMax", value)}
              error={errors.repetitionsMax}
              compact
            />
          </View>
        </View>
      ...
      <Card style={styles.summary}>
        ...
        <Text style={styles.summaryValue}>
          {draft.sets || "—"} séries · {draft.repetitionsMin || "—"}–
          {draft.repetitionsMax || "—"} reps · {draft.restSeconds || "—"}s
        </Text>
        <Text style={styles.summaryCopy}>
          Presets e exercícios já salvos mantêm suas configurações atuais.
        </Text>
      </Card>

      <Button
        disabled={hasPrescriptionErrors(errors)}
      ...
      {saved ? (
        ...
        >
          Tudo certo. Seus novos padrões já estão ativos.
        </Text>
      ) : null}
    ...
    <View style={styles.field}>
      ...
      <View style={[styles.stepper, error && styles.inputError]}>
        <Pressable
          ...
          onPress={() => adjust(-1)}
          style={({ pressed }) => [
            compact ? styles.compactButton : styles.stepButton,
...
const styles = StyleSheet.create({
  ...
  intro: { ...typography.bodyLG, color: colors.textSecondary, lineHeight: 24 },
  card: { gap: spacing.lg },
  sectionHeader: {
    flexDirection: "row",
    alignItems: "center",
  ...
  markText: { ...typography.headingMD, color: colors.primary },
  ...
  stepButton: { width: 58, alignItems: "center", justifyContent: "center" },
  compactButton: { width: 38, alignItems: "center", justifyContent: "center" },
  pressed: { backgroundColor: "#3A1D0C" },
  stepText: { ...typography.headingLG, color: colors.primary },
  input: {
  ...
  rangeRow: { flexDirection: "row", alignItems: "flex-start", gap: spacing.sm },
  rangeField: { flex: 1 },
  ...
  summaryValue: { ...typography.headingMD, color: colors.textPrimary },
  summaryCopy: {
    ...typography.bodyMD,
    color: colors.textSecondary,
    lineHeight: 21,
  },
  success: { ...typography.bodyMD, color: colors.success, textAlign: "center" },
