...
public static class StudentOnboardingEndpointExtensions
{
    public static void MapStudentOnboardingApi(this WebApplication app)
    {
        ...
        api.MapGet("/invite-preview", async (string? code, PersonalUltraDbContext db, TimeProvider clock, HttpContext context, CancellationToken ct) =>
        {
            var normalizedCode = NormalizeCode(code);
            var trainerName = await db.StudentInvites.AsNoTracking()
                .Where(item => item.InviteCode == normalizedCode && item.AcceptedAt == null && item.ExpiresAt > clock.GetUtcNow())
                .Select(item => item.Trainer.Name)
                .SingleOrDefaultAsync(ct);
            return trainerName is null
                ? ApiEndpointExtensions.ApiError(context, "INVITE_NOT_FOUND", "Este código de convite não está disponível.", 404)
                : Results.Ok(new InvitePreviewResponse(trainerName));
        });
    ...
    private static string NormalizeCode(string? value) => new((value ?? "").Where(char.IsDigit).ToArray());
