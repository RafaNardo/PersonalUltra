...
public sealed record StudentWorkoutPreviewResponse(Guid Id, string Name, string Notes, int RecommendedDay, bool IsRecommended, string State, Guid? ActiveSessionId, DateTimeOffset? LastCompletedAt, IReadOnlyList<StudentWorkoutExercisePreview> Exercises);
...
public sealed record StudentSessionExerciseDetail(Guid Id, Guid? ExerciseId, string Name, string? PrimaryMuscleGroup, string? Equipment, string? ImageRef, string? Instructions, int Sequence, int Sets, int RepetitionsMin, int RepetitionsMax, int RestSeconds, string Notes, int CompletedSets, IReadOnlyList<StudentSetPerformance> Performances);
