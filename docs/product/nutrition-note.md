# Nutrition — Product Note

Nutrição permanece na V1 demo porque é importante para o produto.

Antes de produção, o fluxo deve ser validado do ponto de vista jurídico/regulatório sobre quem pode prescrever planos alimentares individualizados.

## Regras de produto agora
- não depender apenas de disclaimer como estratégia de compliance;
- manter rastreabilidade de quem criou/alterou o plano;
- deixar o domínio preparado para representar profissional responsável no futuro;
- IA não cria nem altera prescrição alimentar na V1;
- dúvidas sobre o plano podem ser tratadas no chat humano com o Trainer.

Revisão legal/produto é obrigatória antes do lançamento de produção.

A decisão demo-first atual, seus limites e as referências pesquisadas estão em
`docs/design/nutrition-experience-review.md`. A ação de salvar disponibiliza o
documento persistido ao Student; a demo não modela rascunhos ou versões.
